...
export default function AddTechModal({ caseStudyId, onClose }) {
 ...
 // Inside your AddTechModal component
  const handleImage = (e) => {
  ...
  try {
    ...
    if (res.status === 422) {
      // ⭐ THIS WILL TELL YOU EXACTLY WHAT THE BACKEND HATES
      console.error("Validation Errors:", data.errors); 
      toast(data.message || "Check your input fields", "error");
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-[2.5rem] w-full max-w-lg p-8 shadow-2xl">
        ...
        <div className="space-y-5">
           ...
           <input placeholder="Tech Name (e.g. React)" className="w-full border-2 border-gray-100 p-4 rounded-2xl outline-none focus:border-indigo-500 font-bold"
            onChange={e =>
             setForm({...form, name: e.target.value})
             }
              />
             <input placeholder="Category (e.g. Frontend , Backend)" className="w-full border-2 border-gray-100 p-4 rounded-2xl outline-none focus:border-indigo-500 font-bold"
              onChange={e =>
