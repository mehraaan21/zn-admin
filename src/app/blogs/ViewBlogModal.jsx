...
import { X, Newspaper, FileText, Search, Tag, Calendar, Globe, Eye } from "lucide-react";
        ...
        >
          ...
          <div className="relative p-6 border-b bg-gradient-to-r from-slate-50 to-white flex justify-between items-center overflow-hidden">
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div>
                <h2 className="text-xl font-extrabold text-slate-800 tracking-tight">Blog Preview</h2>
                <p className="text-[10px] font-mono text-slate-400 uppercase tracking-widest">ID: #{data.id}</p>
              </div>
            </div>
            <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-400 hover:text-slate-600 cursor-pointer">
              <X size={24} />
            </button>
          </div>
          ...
          <div className="flex border-b bg-slate-50/50 px-6">
            <button
              ...
              className={`flex items-center gap-2 py-4 px-4 text-sm font-bold border-b-2 transition-all ${
                activeTab === "content" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-400"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 py-4 px-4 text-sm font-bold border-b-2 transition-all ${
                activeTab === "seo" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-400"
              }`}
            >
            ...
            {activeTab === "content" ? (
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                ...
                <div className="relative aspect-video rounded-3xl overflow-hidden border-4 border-white shadow-xl bg-slate-100 group">
                  <Image src={data.image_url || "/placeholder.png"} alt={data.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-4 text-xs font-bold text-slate-400 uppercase tracking-tighter">
                    <span className="flex items-center gap-1.5"><Calendar size={14} className="text-blue-500" /> Jan 27, 2026</span>
                    <span className="flex items-center gap-1.5"><Globe size={14} className="text-blue-500" /> {data.slug}</span>
                  </div>
              ...
              <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-black text-blue-500 uppercase mb-2">Meta Title</p>
                    <p className="text-sm font-bold text-slate-700">{data.seo_title || "Not Set"}</p>
                  </div>
                  <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-black text-blue-500 uppercase mb-2">Meta Keywords</p>
                    <div className="flex flex-wrap gap-2">
                      {data.seo_keywords?.split(',').map((tag, i) => (
                        <span key={i} className="px-2 py-0.5 bg-white border border-slate-200 rounded text-[10px] font-bold text-slate-500">{tag.trim()}</span>
                      )) || "None"}
                    </div>
                  </div>
                </div>
                <div className="p-5 bg-slate-900 rounded-2xl border border-slate-800 shadow-inner">
                  <p className="text-[10px] font-black text-slate-400 uppercase mb-3 tracking-widest">Search Result Snippet</p>
                  <p className="text-xs text-blue-400 font-medium mb-1 truncate">{`https://znsoftech.com/blogs/${data.slug}`}</p>
                  <p className="text-sm font-bold text-white mb-2 leading-tight">{data.seo_title || data.title}</p>
                  <p className="text-xs text-slate-400 leading-relaxed line-clamp-3 italic">
                    {data.seo_description || "No SEO description provided. Search engines will pull from main content."}
                  </p>
                </div>
              </motion.div>
            )}
